...
**Aktuální:** v1.0.0  
**Poslední změna:** 3. 6. 2026  
**Git tag:** v0.18.0 (Design A photos: tag `design-A-photos`)

### Historie verzí

| Verze | Datum | Co přibylo |
|---|---|---|
| **v1.0.0** | **3. 6. 2026** | **🎯 Brand finalizace — placeholdery pryč, oficiální brand manuál nasazený.** Major milestone: aplikace běží na finálních hodnotách z [brand.grandpadel.cz](https://www.brand.grandpadel.cz/) v1.1.119 (lokální archiv `vizual/manual-final-2026-05-23/`, 62 souborů, 53 MB). **Barvy** — `#8C1325 → #801928` (Brand Red), `#0A0A0A → #0D0D0D` (měkká černá), `#F2EDE4 → #F8F6F1` (slonová kost, předtím Elephant White), žlutý akcent vyřazen (manuál ho neobsahuje). **Fonty** — Poppins a Pacifico pryč, **Bandeja** (nadpisy) + **Mluvka** (text) od Fungi Type, custom typeface vyrobený na míru pro Grand Padel, self-host přes `next/font/local` (woff2 v `src/app/fonts/`, full OTF rodiny v `public/fonts/`). **Loga** — finální SVG s průhledným pozadím v 18 variantách (primární + GP × bílá/černá/červená × PNG/SVG/EPS), umístěná v `public/logos/` s web-safe názvy. Workaround `make-logo-transparent.mjs` + `public/logos-transparent/` smazán jako obsolete. **Komponenty** — `brand.ts` centrálně refaktorován, `globals.css` CSS proměnné synchronizované, Navbar má SVG loga s `priority`, layout favicon přepnut, hover utility `hover:text-brand` místo hardcoded HEX, prihlášení page má Suspense wrapper (Next 16 requirement) + aktualizované barvy. **PDF/PPTX šablony** — `PrezentacePdf.tsx`, `PrezentacePdfB.tsx`, `PrezentacePptx.ts` přepsané na Bandeja+Mluvka, BG dynamic z `brand.colors.red`, dead code wordmark text fallbacků (z placeholder éry) odstraněn. **Bonus fixes během build testu:** přidán chybějící typ `BrandDesign = "A" \| "B"` v `brand.ts`, `revalidateTag` upgrade na Next 16 dvouargumentovou signaturu (profile "max"), `FotoStyle` přetypován z rozbitého `Parameters<typeof Image>` na `React.ComponentProps<typeof Image>`, `pptx.shapes.RECTANGLE/LINE` → `"rect"/"line" as never` (instance metody v této verzi pptxgenjs neexistují). **`@fontsource/poppins`** dep odstraněna. Production build ✓ 0 errors, 20/20 statických stránek vygenerovaných. Detail v deníku [`denik/2026-06-03-brand-manual-finalizace.md`](../../denik/2026-06-03-brand-manual-finalizace.md). |
| v0.26.0 | 1. 6. 2026 | **Rezervační systém FÁZE 3 — zápis rezervace.** Klik na slot v `/rezervace/dostupnost` → `/rezervace/dostupnost/nova?kurt=…&datum=…&zacatek=…&delka=…` (server component s rekapitulací). Server-side validace v `src/lib/rezervace-actions.ts` (Server Action `vytvorRezervaci`): auth, kurt aktivní + pobočka Olomouc, datum v rozsahu 0–14 dní, **znovu spočítá `vypocitejVolneSloty` proti čerstvým datům z DB** a ověří, že kandidát je ve výsledku (eliminuje race conditions a obejití klienta), pak INSERT do `rezervace` se `stav='potvrzena'` a redirect na `/rezervace/dostupnost/nova/[id]` (potvrzovací stránka). Auth flow: `/prihlaseni?next=…` + `/auth/callback?next=…` s open-redirect filtrem (povolené jen relativní cesty). Bez ceny, bez platby, bez e-mailu — to jsou další fáze. |
| v0.25.1 | 1. 6. 2026 | **Linknout dostupnost z marketingové stránky + DB konsolidace Olomouc.** Na `/rezervace` přibyla mezi hero a „Jak rezervace funguje" sekce s CTA „Zobrazit volné kurty" → `/rezervace/dostupnost`. Migrace `instrukce/migrace/2026-06-01-konsolidace-olomouc-a-aktivace.sql`: sloučení dvou duplicitních olomouckých poboček (placeholder `11111111-…` smazán, otevírací doba migrována na reálnou `dd2d9351-…`), aktivace pobočky i 7 kurtů (`je_aktivni=true`). Bez aktivních kurtů by stránka jen ukazovala „nejsou žádné aktivní kurty". |
| v0.25.0 | 1. 6. 2026 | **Rezervační systém FÁZE 2 — zobrazení dostupnosti (čtení, žádný zápis).** Nová samostatná routa `/rezervace/dostupnost` (původní marketingová `/rezervace` nedotčená). Formulář: datum (max +14 dní), délka hry (60/90/120/180/240/300/360), část dne (ráno/poledne/odpoledne/večer/kdykoliv). Klientská komponenta čte přes `@/lib/supabase/client`: pobočky v Olomouci → aktivní kurty (`je_aktivni=true`) → potvrzené rezervace na daný den. Logika je oddělená v `src/lib/dostupnost.ts` (čistá funkce `vypocitejVolneSloty`) — respektuje otevírací dobu 7:00–23:00, kroky po 30 min, **pravidlo mezer** (mezi kandidátem a sousední rezervací mezera 0 nebo ≥60 min; 30min mezery povolené jen na začátku / konci dne nebo pod 32h horizontem), prázdný kurt + 32h pravidlo. Pobočka se hledá podle `mesto='Olomouc'` (žádné UUID hardcoded). Časová zóna Europe/Prague řešená helpery `pulnocPrahaToUTC` / `minutyOdPulnociPraha`. Unit testy `scripts/test-dostupnost.mjs` (30 OK / 0 FAIL) pokrývají všechny edge case. Žádný zápis rezervace, žádná platba, žádná cena. |
| v0.24.0 | 26. 5. 2026 | **Logo PNG s průhledným pozadím — sjednocení Brand Red `#8C1325` napříč.** Konsolidace dlouhé série oprav z dneška. **(1) Sharp skript** `scripts/make-logo-transparent.mjs` zpracoval `vizual/loga/Grand Padel logo GP.png` a `Grand Padel název logo.png` — bordó pozadí (`#801A28` ± 35 RGB) nahrazeno transparentními pixely. Výstup do `public/logos-transparent/gp-monogram.png` (47KB) a `gp-full.png` (86KB). Kopie taky do `vizual/loga/gp-{monogram,full}-transparent.png`. 72–78% pixelů transparentních. **(2) Replace `#801A28` → `#8C1325`** ve 21 souborech `src/` (UTF-8 safe přes .NET File IO, nikoli PowerShell Set-Content který rozbil diakritiku). Brand Red sjednocen napříč CSS, PDF, PPTX, brand.ts. **(3) Design B krátké varianty portrét A4** — `PrezentacePdfB.tsx` + `PrezentacePptx.ts`. Levý strip s transparentním GP monogramem + vertikální čára 2pt + horizontální čára dole 2pt. BIG "Grand Padel" wordmark (transparentní PNG, ne text) centrovaný nahoře — na 1p a 1. stránce 2p verze. Menší wordmark na 2. stránce 2p. Fotky (hero + teambuilding) NA KONCI prezentace. Tagline "{firma} & Grand Padel — spolupráce, která dává smysl." centrovaný dole. **(4) Slug → label mapování** v `typyLabel()` — "firemni_turnaj" → "Firemní turnaj / teambuilding" napříč všemi PDF/PPTX. **(5) PPTX fix overlapping text** — `bulletsBPortrait` `lineH = 0.75"` (z 0.45"), `valign: "top"`, dynamický posun CTA+footer podle skutečné délky bulletů (vrací konečnou Y pozici). Limit 4 hodnoty + 4 návrhy. **(6) PPTX layout přepínač** dle `delka` query paramu (`?delka=1p\|2p`), `defineLayout({ name: "GP_PORTRAIT", width: 8.27, height: 11.69 })` pro krátké varianty. **(7) Pacifico font** stažen do `public/fonts/pacifico/Pacifico-Regular.ttf` jako placeholder skript pro "Grand" (zatím nepoužíván — vyhrazeno pro budoucí text-based logo). **(8) Dokumentace** — nový soubor `vizual/loga/PRAVIDLA-POUZITI.md` s plnou story barevného nesouladu a postupem opravy. CLAUDE.md doplněn o "Logo na barevném pozadí — KRITICKÉ" pravidlo. `paleta.md` má varování nad statusem. Deník `2026-05-26-logo-barva-lekce.md`. |
| v0.23.0 | 26. 5. 2026 | **Rychlé založení + Master AI sekce + nullable metadata.** SQL migrace: `firma_kontakt_email`, `typy_spoluprace`, `lokalita`, `velikost_firmy` všechny nullable + uvolněné CHECK constrainty (přijímají NULL). `/admin/prezentace` má nahoře client komponent `RychleZalozeni` — input "Název firmy" + tlačítko → POST `/api/admin/prezentace/ulozit` s jen `firmaNazev` → redirect na detail. `ulozit` route validace uvolněna (firmaNazev jediné povinné, typy/lokalita/velikost volitelné). Detail stránka má novou sekci `MasterAiSekce` — hotový velký prompt pro Claude.ai který dohledá vše o firmě (lokalita, velikost, typ, brand barvy, obsah) → vrátí JSON → user vlepí zpět → server PATCHne všechna pole najednou přes `/api/admin/prezentace/[id]/aktualizovat` endpoint. |
| v0.22.0 | 25. 5. 2026 | **Krátké varianty PDF (1p + 2p) — první iterace landscape.** Příprava infrastruktury: `delka` query param na `/pdf` a `/pptx`, prop `delka` v `PrezentacePdf` a `PrezentacePdfB`, `renderOneePageA`/`renderTwoPageA`/`renderOnePageB`/`renderTwoPageB` funkce. Detail stránka má 3 sekce po 4 tlačítkách (Celá / 2 stránky / 1 stránka × PDF A/B + PPTX A/B). PPTX: `pptxgenjs` rozšířený o `sestavitDesignA1Page`/`sestavitDesignA2Pages`/`sestavitDesignB1Page`/`sestavitDesignB2Pages`. Krátké varianty vždy `bez_cen` ("Konkrétní nabídku zpracujeme po setkání"). _Tato iterace byla landscape — v 0.24.0 přepsáno na portrét A4 podle uživatelského feedbacku._ |
| v0.22.x WIP | 24. 5. 2026 | **🚧 Design webu A/B přepínač (rozpracováno, ne autorem Claudem).** Paralelní práce k prezentačním designům. Změny: `lib/brand.ts` rozšířen o kompletní design tokeny — `fontSizes` (h1-caption), `fontWeights`, `lineHeights`, `letterSpacing`, `spacing` (xs-3xl), `radius`, `shadows`, plus `redDeep`/`redDark` barvy. Fonty teď používají `var(--font-poppins)` (CSS proměnná). Nový type `BrandDesign = "A" \| "B"`. `NavbarAuth.tsx` přepsán na CSS vars (`var(--brand)`, `var(--text-muted)`, `var(--border-subtle)`). `admin/page.tsx` má novou dlaždici "Design webu" → odkaz `/admin/design` (přepínač A/B). `admin/layout.tsx` má wrapper `<div data-design="A">` s `var(--bg-page)` + `var(--text-primary)` — admin sekce vždy Design A (i když globálně aktivní B). **Chybí (k doplnění): stránka `/admin/design`, CSS proměnné definice (asi `globals.css`), případně DB sloupec pro uložení aktivního designu.** |
| v0.21.2 | 24. 5. 2026 | **Fix slide 10 (Děkujeme) v Design A** — překrytí mezi "Děkujeme." a podtitulem opraveno (přidaný spacer 24pt v PDF + posun textu níže v PPTX). Sjednocené znění "{firma} & Grand Padel — spolupráce, která dává smysl." napříč všemi exporty A+B. |
| v0.21.1 | 24. 5. 2026 | **Design B fotky vidět + Galerie slide.** Slide 7 Title Sponsorship přepracován na split layout (text vlevo, čistá fotka `centerVstup` vpravo, žádný overlay). Slide 8 Atmosféra nahrazen Galerií — 3 čisté fotky vedle sebe (`akce`, `teambuilding`, `hero`) s popisky pod každou. Slide 10 Děkujeme oprava překryvu + nové znění. Stejně v PDF i PPTX. |
| v0.21.0 | 24. 5. 2026 | **PPTX export (KROK 7).** `pptxgenjs@4.0.1`. `lib/pptx/PrezentacePptx.ts` — imperativní generator pro Design A i B, 9–10 slidů. Endpoint `GET /api/admin/prezentace/[id]/pptx?design=A\|B`. Detail stránka má teď 4 tlačítka: PDF — A, PDF — B, PPTX — A, PPTX — B. Roman může stáhnout PPTX, otevřít v PowerPointu, upravit text/fotky, save as PDF. |
| v0.20.0 | 24. 5. 2026 | **Design B PDF — bordó-dominantní variant.** Inspirace landing grandpadel.cz. `lib/pdf/PrezentacePdfB.tsx` — full bordó pozadí na každém slidu, GP monogram vlevo v stripu, vertikální + horizontální bílé čáry, big Poppins Black typografie. 10 slidů včetně Title Sponsorship a Atmosféra fotky s bordó overlay. PDF route přijímá `?design=A\|B` query param, vrací příslušnou variantu. Detail stránka: dvě tlačítka "PDF — A" a "PDF — B" pro porovnání. |
| v0.19.5 | 24. 5. 2026 | **Duální brand barvy partnera.** Migrace: `firma_barva_sekundarni text`. Wizard krok 4 má 2 vstupní pole — Primární + Sekundární brand barva. Center kurt vstup prompt rozděluje: primární barva → rám vstupu + horní mřížka, sekundární → páska na síti (fallback na primární když není). Příklad pro Rohlík: žlutá #F9D71C + zelená #7AB800. |
| v0.19.4 | 24. 5. 2026 | **Komprese fotek při uploadu (Sharp).** Upload endpoint resize na max 1920×1080 + JPEG quality 82 (mozjpeg). Z 5–8 MB fotek se stanou ~300–500 KB. PDF tím spadl z 36 MB na 2–4 MB. EXIF orientace respektována. Vše ukládá jako .jpg. **Design A photos snapshot** uložen do `instrukce/design-snapshots/design-A-photos.md` + git tag `design-A-photos`. .gitignore přidán pro `/public/photos/prezentace/*/` (per-prezentace runtime data). |
| v0.19.3 | 24. 5. 2026 | **CENTER kurt — druhý prompt: vstupní brána s partner-barvami.** Nový slot `centerVstup` v PHOTO_SLOTS — close-up záběr na vstupní dveře CENTER kurtu, žádní hráči, fokus na partnerské barvy (rám dveří, horní mřížka, páska na síti) + logo na koberci. Existující slot `center` zůstává jako širší architektonický záběr s logem na koberci + zadní skleněné stěně. Slide 7 v PDF přejmenován z "Atmosféra" na "Vaše Title Sponsorship Experience — {Firma} na CENTER kurtu" s collage obou center fotek + akce + teambuilding. |
| v0.19.2 | 24. 5. 2026 | **Upload limit zvýšen na 10 MB.** Stejné jen mírné rozšíření. |
| v0.19.1 | 24. 5. 2026 | **Brand barva partnera ve wizardu.** Migrace: `firma_barva text` (NULL default). Wizard krok 4 (Firma) má nové pole "Brand barva partnera" (placeholder "oranžová #FF6600"). Detail stránka načte barvu a předá ji do FotoSekce — komponenta automaticky substituuje `{PARTNER_COLOR}` v promptu pro CENTER kurt fotku. |
| v0.19.0 | 24. 5. 2026 | **Per-prezentace fotky — upload UI (KROK fotky).** `lib/photo-prompts.ts` — 6 slotů s prompty pro Gemini (hero, akce, center, teambuilding, detail; centerVstup přidán v 0.19.3). Endpoint `POST/DELETE /api/admin/prezentace/[id]/foto` — multipart upload, ukládá do `public/photos/prezentace/{id}/{slot}.jpg`. Klient komponenta `FotoSekce` na detail stránce — 5 (nyní 6) karet s prompt copy, "Otevřít Gemini" odkazem, file input, náhled, "Zobrazit prompt", smazat. PDF route zkouší nejprve per-prezentace fotku, jinak globální fallback z `/public/photos/prezentace/{slot}.jpg`. Globální fotky pořízeny manuálně z Gemini, uloženy jako `hero-kurt.jpg`, `akce-hraci.jpg`, atd. |
| v0.18.0 | 23. 5. 2026 | **PDF export prezentací (KROK 6).** `@react-pdf/renderer@4.5`. `lib/pdf/PrezentacePdf.tsx` — Document s 8 snímky (16:9 widescreen, 960×540pt) v brand identitě: Cover (bordó, logo, firma), O Grand Padelu (3 fakta), Naše centra (Olomouc/Ostrava/Praha s termíny), Hodnota, Konkrétní návrhy, Cenové balíčky (nebo „Individuální nabídka" když bez_cen), Výzva k akci + kontakt, Děkujeme. Poppins font registrován z google/fonts repo přes jsdelivr CDN (váhy 400/500/600/700/900). Endpoint `GET /api/admin/prezentace/[id]/pdf` (Node runtime, renderToBuffer, logo z `/public/gp-logo-full.png` přes base64). Tlačítko "Stáhnout PDF" na detail stránce |
| v0.17.1 | 23. 5. 2026 | **Wizard → promptovací mód (KROK 4 přepracován).** AI integrace přes Anthropic API zrušena (uživatel platil ~$0.02/prezentace). Nový tok: krok 7 ukazuje (1) hotový prompt pro Claude.ai zkopírovat, (2) textarea pro JSON odpověď + tlačítko Načíst, (3) editovatelná pole (úvod / hodnota / návrhy / balíčky / CTA / poznámka). Parser čistí markdown code fence a extrahuje JSON. Endpoint `/api/admin/prezentace/ulozit` (jen INSERT, žádné AI volání). Lib `lib/anthropic.ts` smazán, `.env.local` GP_ANTHROPIC_API_KEY zůstává nepotřebný. Cena: $0 |
| v0.17.0 | 23. 5. 2026 | **AI generace přes Claude API (KROK 4, později nahrazeno v0.17.1).** Anthropic SDK, model `claude-sonnet-4-5`, tool_use pro strukturovaný JSON, prompt caching. Endpoint `/api/admin/prezentace/generovat`. Brand vibe + cenové rámce v system promptu. Bug fix: `ANTHROPIC_API_KEY` kolize se systémovou env proměnnou (Claude Code CLI), přejmenováno na `GP_ANTHROPIC_API_KEY`. RLS bug fix: `je_management()` funkce potřebovala `GRANT EXECUTE TO authenticated` (security cleanup ji omylem zrevokoval) |
| v0.16.1 | 23. 5. 2026 | **Wizard přepořádán a kontakty volitelné.** Nové pořadí kroků: 1) Typ spolupráce, 2) Lokalita, 3) Velikost firmy, 4) Firma+kontakt, 5) Ceny, 6) Doplňující info, 7) Náhled+generování. Jediné povinné pole na firmě = název. Kontakt (jméno, pozice, email, telefon, web) zcela volitelný. DB: `ALTER TABLE prezentace ALTER COLUMN firma_kontakt_email DROP NOT NULL` |
| v0.16.0 | 23. 5. 2026 | **Nový subprojekt: Aplikace pro prezentace pro partnery (KROKY 1–3).** Migrace `2026-05-23-prezentace-init.sql` — 3 tabulky: `prezentace`, `prezentace_views` (anon insert pro public tracking), `cenove_balicky_sablony` + RLS (`management` role přes helper `je_management()`). Stránky `/admin` (server layout s auth check), `/admin/prezentace` (seznam), `/admin/prezentace/nova` (7-krokový wizard s validací), `/admin/prezentace/[id]` (detail s vygenerovaným obsahem). `lib/brand.ts` — JEDEN zdroj brand tokenů (Brand Red `#8C1325`, Poppins, konstanty TYPY/LOKALITY/VELIKOSTI). NavbarAuth rozšířen — "Admin" link viditelný jen pro management. Detail prezentace s metadaty (firma, kontakt, typ, lokalita, velikost) + sekcemi (úvod, hodnota, návrhy, balíčky, CTA, poznámka). `CLAUDE.md` doplněn: zásada "README a CLAUDE.md jsou rozcestníky, ne snapshots" (zdroj pravdy = paleta.md / fonty.md / stav-projektu.md). `vizual/README.md` přepsán jako čistý rozcestník |
